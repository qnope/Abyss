...
void main() {
  ...
  Map<UnitType, Unit> makeUnits({int scouts = 3, int harpoonists = 2}) {
    return {
      if (scouts > 0)
        UnitType.scout: Unit(type: UnitType.scout, count: scouts),
      if (harpoonists > 0)
        UnitType.harpoonist:
  ...
  group('ReinforcementDialog', () {
    ...
    testWidgets('displays info banner with transit time', (tester) async {
      await openDialog(tester);
      expect(
        find.textContaining('Les renforts arriveront au prochain tour'),
        findsOneWidget,
      );
    ...
    testWidgets('Annuler closes dialog without calling onConfirm',
      ...
      expect(find.byType(ReinforcementDialog), findsNothing);
    });
  });
}
